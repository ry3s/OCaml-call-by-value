...
/*  | LET REC var pattern_expr EQ expr           { CRLet ($3,"x", EMatch( EVar "x", [($4, $6)])) }  */
  | LET REC var var argument_expr              { CRLet ($3, $4, $5) }
  | LET REC var var EQ expr AND mutualrec_expr { CMRLet ([($3, $4, $6)] @ $8) }
  | expr                                       { CExp $1 }
;
mutualrec_expr:
  | var var EQ expr AND mutualrec_expr  { [($1, $2, $4)] @ $6 }
...
branch_expr:
  ...
  | cons_pattern_expr RARROW expr                 { [($1, $3)] }
;

cons_pattern_expr:
  | cons_pattern_expr CONS pattern_expr { PCons ($1, $3) }
  | pattern_expr                        { $1 }
;
